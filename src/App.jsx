...
const Dashboard = ({ data }) => (
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
      ...
      <KPICard label="Personalkosten" value={`€${(data.kpis.costs / 1000).toFixed(0)}K`} sub="Anteil: 71.8%" color={COLORS.gold} icon="👥" />
      <KPICard label="EBIT-Marge" value={`${((data.kpis.profit / data.kpis.revenue) * 100).toFixed(1)}%`} sub="€350K Gewinn" color={COLORS.green} icon="📈" />
      <KPICard label="Mitarbeiterzahl" value={data.kpis.headcount} sub="6 offene Stellen" color={COLORS.accent2} icon="🏢" />
  ...
  {selected && (
    <div className="card animate-in" style={{ position: "sticky", top: 0, height: "fit-content" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", color: COLORS.muted, cursor: "pointer", fontSize: 16 }}>✕</button>
      </div>
